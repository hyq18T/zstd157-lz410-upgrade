...
保证“脚本声称的内核”和“二进制要求的那个内核”永远一致。

**校验粒度只到内核版本开头**（例如 `6.6.118`，即 `uname -r` 第一个 `-` 之前的部分）：

- 开头不同 → 拦下（这份 `.ko` 根本不是给这个内核编的），报错里同时打印两个完整串，
  并附上按当前内核重编的命令；
- 开头相同、后面的构建号不同 → **放行但打印警告**，能不能 `insmod` 交给内核判定；
  真被拒时 `zram-algo.sh` 会按 refcnt 发现“新驱动没在服务”，不会假装成功。

另外两种情况也分别给解释：包不完整（`.ko` 读不到，提示校验 sha256）、
刷入环境异常（`uname -r` 读不出来）。
